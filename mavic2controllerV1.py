...
class Mavic (Robot):
    ...
    K_VERTICAL_THRUST = 67.5  # with this thrust, the drone lifts.
    # Vertical offset where the robot actually targets to stabilize itself.
    K_VERTICAL_OFFSET = 0.6
    K_VERTICAL_P = 2.0        # P constant of the vertical PID.
    K_ROLL_P = 25.0           # P constant of the roll PID.
    ...
    def run(self):
        ...
        print(f"State: {state} - Target altitude: {self.target_altitude}m")
        ...
        while self.step(self.time_step) != -1:
            ...
            if start_x_pos is None:
                start_x_pos = x_pos

            if state == "TAKEOFF":
                if altitude > self.target_altitude - 0.2:
                    state = "MOVE_FORWARD"
                    print(f"State: {state} - Moving forward")

            elif state == "MOVE_FORWARD":
                target_x = start_x_pos + 3.0
                x_error = target_x - x_pos
                
                pitch_disturbance = clamp(x_error * -0.1, self.MAX_PITCH_DISTURBANCE, 0.1)

                if abs(x_error) < self.target_precision: 
                    state = "LAND"
                    self.target_altitude = 0.1  
                    pitch_disturbance = 0       
                    print(f"State: {state} - Landing")

            elif state == "LAND":
                if altitude < 0.2: 
                    state = "OFF"
                    print(f"State: {state} - Landed")
            ...
            if state != "OFF":
                ...
                vertical_input = self.K_VERTICAL_P * clamped_difference_altitude
